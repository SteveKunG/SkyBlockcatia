...
public class SkyBlockAPIUtils
{
    ...
    public static void setApiKey()
    {
        ...
        SkyBlockAPIUtils.API_KEY = SkyBlockcatiaConfig.GENERAL.hypixelApiKey.get();
    ...
    public static List<ItemStack> decodeItem(JsonObject currentProfile, SkyBlockInventoryType type)
    {
        if (currentProfile.has(type.getApiName()))
        {
            ...
            try
            {
                ...
                for (int i = type == SkyBlockInventoryType.INVENTORY ? 9 : 0; i < list.size(); ++i)
                {
                    itemStack.add(SkyBlockAPIUtils.flatteningItemStack(list.getCompound(i)));
                }
