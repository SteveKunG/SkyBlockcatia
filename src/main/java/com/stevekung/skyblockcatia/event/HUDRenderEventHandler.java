...
public class HUDRenderEventHandler
{
    ...
    public void onClientBlockBreak(ClientBlockBreakEvent event)
    {
        ...
        if (HypixelEventHandler.isSkyBlock && !this.mc.player.getHeldItemMainhand().isEmpty() && this.mc.player.getHeldItemMainhand().hasTag() && (this.mc.player.getHeldItemMainhand().getTag().getCompound("ExtraAttributes").getString("id").equals("JUNGLE_AXE") || this.mc.player.getHeldItemMainhand().getTag().getCompound("ExtraAttributes").getString("id").equals("TREECAPITATOR_AXE")))
        {
    ...
    public void onPreInfoRender(RenderGameOverlayEvent.Pre event)
    {
        ...
        if (ExtendedConfig.INSTANCE.axeCooldown)
        {
        ...
        if (event.getType() == RenderGameOverlayEvent.ElementType.TEXT)
        {
            ...
            if (ExtendedConfig.INSTANCE.axeCooldown && jungleAxeDelay >= 0.01D)
            {
                crosshairInfo.add(new CrosshairOverlay(ExtendedConfig.INSTANCE.axeCooldownColor, jungleAxeDelay));
            }
