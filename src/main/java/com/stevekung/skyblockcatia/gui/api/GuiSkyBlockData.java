...
public class GuiSkyBlockData extends Screen
{
    ...
    private SkyBlockFakePlayerEntity player;
    private String skillAvg;

    // Info & Inventory
    private static final int SIZE = 36;
    private static final Inventory TEMP_INVENTORY = new Inventory(GuiSkyBlockData.SIZE);
    private static final Inventory TEMP_ARMOR_INVENTORY = new Inventory(4);
    static final List<SkyBlockInventory> SKYBLOCK_INV = new ArrayList<>();
    private int selectedTabIndex = SkyBlockInventoryGroup.INVENTORY.getIndex();
    ...
    public void init()
    {
        ...
        this.addButton(this.doneButton = new Button(this.width / 2 - 154, this.height - 25, 150, 20, LangUtils.translate("gui.close"), button -> this.minecraft.displayGuiScreen(this.error ? new GuiSkyBlockAPIViewer(GuiSkyBlockAPIViewer.GuiState.SEARCH, this.username, this.displayName, this.profiles) : null)));
        this.addButton(this.backButton = new Button(this.width / 2 + 4, this.height - 25, 150, 20, LangUtils.translate("gui.back"), button -> this.minecraft.displayGuiScreen(this.profiles.size() == 0 ? new GuiSkyBlockAPIViewer(GuiSkyBlockAPIViewer.GuiState.EMPTY, this.username, this.displayName) : new GuiSkyBlockAPIViewer(GuiSkyBlockAPIViewer.GuiState.SEARCH, this.username, this.displayName, this.profiles))));
        ...
        this.setCurrentGroup(SkyBlockInventoryGroup.GROUPS[i]);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int state)
    {
        ...
        else
        {
            if (state == 0 && this.currentSlot != null && this.currentSlot instanceof EmptyStats && ((EmptyStats)this.currentSlot).getType() == EmptyStats.Type.INVENTORY)
            {
                ...
                for (SkyBlockInventoryGroup group : SkyBlockInventoryGroup.GROUPS)
                {
    ...
    public void render(int mouseX, int mouseY, float partialTicks)
    {
        ...
        if (this.loadingApi)
        {
            ...
            this.drawString(this.font, GuiSkyBlockAPIViewer.downloadingStates[(int)(Util.milliTime() / 500L % GuiSkyBlockAPIViewer.downloadingStates.length)], this.width / 2 + i / 2, this.height / 2 + this.font.FONT_HEIGHT * 2 - 35, 16777215);
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    String total1 = TextFormatting.GRAY + "Unique Minions: " + TextFormatting.YELLOW + this.craftedMinionCount + "/" + GuiSkyBlockData.MAXED_UNIQUE_MINIONS + TextFormatting.GRAY + " (" + this.craftedMinionCount * 100 / GuiSkyBlockData.MAXED_UNIQUE_MINIONS + "%)";
    ...
    private void performedBasicInfo(BasicInfoViewButton basicInfoButton)
    {
        ...
        case INVENTORY:
            ...
            this.setCurrentGroup(SkyBlockInventoryGroup.GROUPS[this.selectedTabIndex]);
            break;
        case COLLECTIONS:
    ...
    private boolean isMouseOverGroup(SkyBlockInventoryGroup group, double mouseX, double mouseY)
    {
    ...
    private void setCurrentGroup(SkyBlockInventoryGroup group)
    {
        if (group == null)
        {
            return;
    ...
    {
        if (SkyBlockInventoryGroup.GROUPS[this.selectedTabIndex] == null)
        {
            return false;
        }
        return SkyBlockInventoryGroup.GROUPS[this.selectedTabIndex].hasScrollbar() && this.skyBlockContainer.canScroll();
    }
    ...
    private boolean renderGroupsHoveringText(SkyBlockInventoryGroup group, int mouseX, int mouseY)
    {
        int i = group.getColumn();
        int j = 28 * i;
        int k = 0;
    ...
    {
        RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
        SkyBlockInventoryGroup group = SkyBlockInventoryGroup.GROUPS[this.selectedTabIndex];

        for (SkyBlockInventoryGroup group1 : SkyBlockInventoryGroup.GROUPS)
        {
    ...
    private void drawGroup(SkyBlockInventoryGroup group)
    {
    ...
    private void getCollections(JsonObject currentProfile)
    {
        ...
        SkyBlockCollection dummyCollection = new SkyBlockCollection(ItemStack.EMPTY, null, -1, -1);
        ...
        if (collections != null)
        {
            ...
            for (Map.Entry<String, JsonElement> collection : collections.getAsJsonObject().entrySet())
            {
                ...
                SkyBlockCollection itemCollection = new SkyBlockCollection(new ItemStack(item), type, collectionCount, level);
            ...
            if (!farming.isEmpty())
            {
                this.collections.add(new SkyBlockCollection(ItemStack.EMPTY, SkyBlockCollection.Type.FARMING, -1, -1));
                this.collections.addAll(farming);
            }
            if (!mining.isEmpty())
            {
                this.collections.add(dummyCollection);
                this.collections.add(new SkyBlockCollection(ItemStack.EMPTY, SkyBlockCollection.Type.MINING, -1, -1));
                this.collections.addAll(mining);
            }
            if (!combat.isEmpty())
            {
                this.collections.add(dummyCollection);
                this.collections.add(new SkyBlockCollection(ItemStack.EMPTY, SkyBlockCollection.Type.COMBAT, -1, -1));
                this.collections.addAll(combat);
            }
            if (!foraging.isEmpty())
            {
                this.collections.add(dummyCollection);
                this.collections.add(new SkyBlockCollection(ItemStack.EMPTY, SkyBlockCollection.Type.FORAGING, -1, -1));
                this.collections.addAll(foraging);
            }
            if (!fishing.isEmpty())
            {
                this.collections.add(dummyCollection);
                this.collections.add(new SkyBlockCollection(ItemStack.EMPTY, SkyBlockCollection.Type.FISHING, -1, -1));
        ...
        if (pets.size() > 0)
        {
            for (JsonElement element : pets)
            {
                ...
                try
                {
                    ...
                    {
                        list.add(StringNBT.valueOf(ITextComponent.Serializer.toJson(new StringTextComponent(TextFormatting.RESET + "" + TextFormatting.GRAY + "Current EXP: " + TextFormatting.YELLOW + FORMAT.format(level.getCurrentPetXp()) + TextFormatting.GOLD + "/" + TextFormatting.YELLOW + NumberUtils.formatWithM(level.getXpRequired())))));
                    }

                    list.add(StringNBT.valueOf(ITextComponent.Serializer.toJson(new StringTextComponent(""))));
                    list.add(StringNBT.valueOf(ITextComponent.Serializer.toJson(new StringTextComponent(TextFormatting.RESET + "" + TextFormatting.GRAY + "Total XP: " + TextFormatting.YELLOW + NumberUtils.format(level.getPetXp()) + TextFormatting.GOLD + "/" + TextFormatting.YELLOW + NumberUtils.formatWithM(level.getTotalPetTypeXp())))));
            ...
            {
                SKYBLOCK_INV.add(new SkyBlockInventory(data.getItemStack(), SkyBlockInventoryGroup.PET));
            }
        }
    }
    ...
    private void getHealthFromCake(CompoundNBT extraAttrib)
    {
        ...
        try
        {
            ...
            {
                itemStack1.add(SkyBlockAPIUtils.flatteningItemStack(list.getCompound(i)));
            }
    ...
    private void getInventories(JsonObject currentProfile)
    {
        ...
        SKYBLOCK_INV.add(new SkyBlockInventory(mainInventory, SkyBlockInventoryGroup.INVENTORY));
        SKYBLOCK_INV.add(new SkyBlockInventory(SkyBlockAPIUtils.decodeItem(currentProfile, SkyBlockInventoryType.ENDER_CHEST), SkyBlockInventoryGroup.ENDER_CHEST));
        SKYBLOCK_INV.add(new SkyBlockInventory(accessoryInventory, SkyBlockInventoryGroup.ACCESSORY));
        SKYBLOCK_INV.add(new SkyBlockInventory(SkyBlockAPIUtils.decodeItem(currentProfile, SkyBlockInventoryType.POTION_BAG), SkyBlockInventoryGroup.POTION));
        SKYBLOCK_INV.add(new SkyBlockInventory(SkyBlockAPIUtils.decodeItem(currentProfile, SkyBlockInventoryType.FISHING_BAG), SkyBlockInventoryGroup.FISHING));
        SKYBLOCK_INV.add(new SkyBlockInventory(SkyBlockAPIUtils.decodeItem(currentProfile, SkyBlockInventoryType.QUIVER), SkyBlockInventoryGroup.QUIVER));
        SKYBLOCK_INV.add(new SkyBlockInventory(SkyBlockAPIUtils.decodeItem(currentProfile, SkyBlockInventoryType.CANDY), SkyBlockInventoryGroup.CANDY));
    ...
    private void createFakePlayer()
    {
        ...
        this.player = new SkyBlockFakePlayerEntity(this.minecraft.world, this.profile);
    ...
    class SkyBlockInventory
    {
        ...
        private final SkyBlockInventoryGroup group;

        public SkyBlockInventory(List<ItemStack> items, SkyBlockInventoryGroup group)
        {
            this.items = items;
            this.group = group;
        }
        ...
        public SkyBlockInventoryGroup getGroup()
        {
            return this.group;
        }
    }
    ...
    static class ArmorContainer extends Container
    {
        ...
        {
            super(null, 0);
            this.addSlot(new Slot(GuiSkyBlockData.TEMP_ARMOR_INVENTORY, 0, -52, 75)); // boots
            this.addSlot(new Slot(GuiSkyBlockData.TEMP_ARMOR_INVENTORY, 1, -52, 56));
            this.addSlot(new Slot(GuiSkyBlockData.TEMP_ARMOR_INVENTORY, 2, -52, 36));
            this.addSlot(new Slot(GuiSkyBlockData.TEMP_ARMOR_INVENTORY, 3, -52, 12)); // helmet
        }
    ...
    static class SkyBlockContainer extends Container
    {
        ...
        public SkyBlockContainer()
        {
            ...
            for (int columns = 0; columns < 4; ++columns)
            {
                ...
                {
                    this.addSlot(new Slot(GuiSkyBlockData.TEMP_INVENTORY, columns * 9 + rows, 12 + rows * 18, 18 + columns * 18));
                }
            }
            this.scrollTo(0.0F);
        }
        ...
        public void scrollTo(float scroll)
        {
            ...
            for (int k = 0; k < 4; ++k)
            {
                for (int l = 0; l < 9; ++l)
                {
                    ...
                    {
                        GuiSkyBlockData.TEMP_INVENTORY.setInventorySlotContents(l + k * 9, ItemStack.EMPTY);
                    }
                }
            }
        }
    ...
    static class EmptyStats extends ScrollingListScreen
    {
        ...
        public EmptyStats(GuiSkyBlockData parent, int width, int height, int top, int bottom, int left, int slotHeight, Type type)
        {
    ...
    class InfoStats extends ScrollingListScreen
    {
        ...
        public InfoStats(GuiSkyBlockData parent, int width, int height, int top, int bottom, int left, int slotHeight, List<SkyBlockInfo> stats)
        {
            super(parent, width, height, top, bottom, left, slotHeight);
            this.stats = stats;
        }
        ...
        protected void drawPanel(int index, int left, int right, int top)
        {
            ...
            this.fontRenderer.drawString(stat.getTitle(), left + 3, top, index % 2 == 0 ? 16777215 : 9474192);
            this.fontRenderer.drawString(stat.getValue(), right - this.fontRenderer.getStringWidth(stat.getValue()) - 10, top, index % 2 == 0 ? 16777215 : 9474192);
        }
    }
    ...
    class SlayerStats extends ScrollingListScreen
    {
        ...
        public SlayerStats(GuiSkyBlockData parent, int width, int height, int top, int bottom, int left, int slotHeight, List<SkyBlockSlayerInfo> stats)
        {
            super(parent, width, height, top, bottom, left, slotHeight);
            this.stats = stats;
        ...
        protected void drawPanel(int index, int left, int right, int top)
        {
            ...
            case XP_AND_MOB:
                if (stat.getText().equals("Zombie"))
                {
                    ...
                    ItemStack helmet = SkyBlockRenderUtils.getSkullItemStack(GuiSkyBlockData.REVENANT_HORROR_HEAD[0], GuiSkyBlockData.REVENANT_HORROR_HEAD[1]);
                    ...
                    GuiSkyBlockData.drawEntityOnScreen(left + 80, top + 60, 40, zombie);
                }
                else if (stat.getText().equals("Spider"))
                {
                    ...
                    GuiSkyBlockData.drawEntityOnScreen(left + 80, top + 40, 40, cave);
                    GuiSkyBlockData.drawEntityOnScreen(left + 80, top + 60, 40, spider);
                ...
                else
                {
                    ...
                    wolf.setAngry(true);
                    GuiSkyBlockData.drawEntityOnScreen(left + 80, top + 60, 40, wolf);
                }
    ...
    class Others extends ScrollingListScreen
    {
        ...
        public Others(GuiSkyBlockData parent, int width, int height, int top, int bottom, int left, int slotHeight, List<SkyBlockStats> stats, SkyBlockStats.Type type)
        {
            super(parent, width, height, top, bottom, left, slotHeight);
    ...
    class SkyBlockCollections extends ScrollingListScreen
    {
        ...
        public SkyBlockCollections(GuiSkyBlockData parent, int width, int height, int top, int bottom, int left, int slotHeight, List<SkyBlockCollection> collection)
        {
            super(parent, width, height, top, bottom, left, slotHeight);
    ...
    class SkyBlockCraftedMinions extends ScrollingListScreen
    {
        ...
        public SkyBlockCraftedMinions(GuiSkyBlockData parent, int width, int height, int top, int bottom, int left, int slotHeight, List<CraftedMinion> craftMinions)
        {
            super(parent, width, height, top, bottom, left, slotHeight);
