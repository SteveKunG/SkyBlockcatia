...
public class HUDRenderEventHandler
{
    ...
    public void onRenderChat(RenderGameOverlayEvent.Chat event)
    {
        if (this.mc.currentScreen != null && this.mc.currentScreen instanceof GuiChest)
        {
            ...
            if (IndicatiaEventHandler.showChat && IndicatiaEventHandler.CHATABLE_LIST.stream().anyMatch(invName -> chest.lowerChestInventory.getDisplayName().getUnformattedText().contains(invName)))
            {
