...
public class GiftToast implements Toast
{
    ...
    public GiftToast(ItemStack itemStack, ToastUtils.DropType rarity, boolean santaGift)
    {
        ...
        this.maxDrawTime = this.itemDrop.type().getTime();
        this.texture = new ResourceLocation("skyblockcatia:textures/gui/gift_toasts_" + (santaGift ? 1 : 1 + new Random().nextInt(2)) + ".png");
    }

    @Override
    public Toast.Visibility render(PoseStack poseStack, ToastComponent toastGui, long delta)
    {
        var drop = this.itemDrop;
        var itemStack = drop.itemStack();
        var itemName = itemStack.getHoverName();
